...
L.Control.HeatmapControl = L.Control.extend({
	...
	onAdd: function (map) {
		...
		this._legendCanvas.height = 20;
		var legend = L.DomUtil.create('div','heatmapLegendImg', this._heatmapControlContainer);
		legend.innerHTML = "<span id='heatmapLegendTitle'></span><br><span id='heatmapLegendMin'></span><span id='heatmapLegendMax'></span><img src='' id='heatmapLegendGradient'>";
	...
	updateLegend: function (data) { // Called each time heatmap's extremas change
        ...
        if (data != this._extremeData) {
		  ...
		  var label = document.querySelector('#heatmapLegendTitle');

	      min.innerHTML = data.min;
	      max.innerHTML = Math.ceil(data.max);
		  label.innerHTML = "Demand Map";
